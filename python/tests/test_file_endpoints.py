...
def sample_file():
    ...
    return ("test_document.txt", b"This is a test document with some content for testing file uploads.")
...
def test_file_upload_increments_chunks_created_metric(get_metrics, upload_file, large_file_content):
...
def test_file_upload_with_case_variant_extension(upload_file):
    ...
    resp = upload_file("uppercase_test.PDF", b"Testing uppercase extension", "application/pdf")
    assert resp.status_code == 400
...
def test_file_upload_chunk_metadata_contains_chunk_index(client, upload_file, large_file_content):
...
def test_file_upload_and_delete_affects_embeddings_count(get_metrics, upload_file, client):
    ...
    assert after_upload["index"]["total_embeddings"] == initial_embeddings + chunks_created
    
    delete_resp = client.delete("/file/delete/embeddings_test.txt")
    assert delete_resp.status_code == 200
    
    after_delete = get_metrics()
    assert after_delete["index"]["deleted_documents"] > initial_metrics["index"]["deleted_documents"]
...
def test_file_operations_consistency_across_multiple_uploads_and_deletes(client, upload_file):
