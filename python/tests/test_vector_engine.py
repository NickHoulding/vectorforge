...
def test_vector_engine_loads_model(vector_engine):
    ...
    assert vector_engine.model.get_sentence_embedding_dimension() == VFGConfig.EMBEDDING_DIMENSION
...
def test_add_doc_with_invalid_source_file_type_raises_error(vector_engine):
    ...
    doc_id = vector_engine.add_doc("Test content", {"source_file": 123, "chunk_index": 0})
...
def test_add_doc_with_invalid_chunk_index_type_raises_error(vector_engine):
    ...
    doc_id = vector_engine.add_doc("Test content", {"source_file": "test.txt", "chunk_index": "0"})
    assert doc_id is not None
...
def test_list_files_ignores_docs_without_source_file(vector_engine):
    ...
    vector_engine.add_doc("Doc with file", {"source_file": "test.txt", "chunk_index": 0})
...
def test_list_files_excludes_deleted_docs(vector_engine):
    ...
    doc_id = vector_engine.add_doc("Content 2", {"source_file": "delete.txt", "chunk_index": 0})
...
def test_delete_file_returns_doc_ids(vector_engine):
    ...
    doc_id1 = vector_engine.add_doc("Chunk 1", {"source_file": "file.txt", "chunk_index": 0})
    doc_id2 = vector_engine.add_doc("Chunk 2", {"source_file": "file.txt", "chunk_index": 1})
...
def test_compaction_triggered_automatically(vector_engine):
    ...
    assert len(vector_engine.deleted_docs) == 0 or vector_engine._should_compact() is False
