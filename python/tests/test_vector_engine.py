...
def test_vector_engine_initialization():
    ...
    assert engine.compaction_threshold == VFGConfig.COMPACTION_THRESHOLD
...
def test_vector_engine_loads_model():
    ...
    assert engine.model_name == VFGConfig.MODEL_NAME
    assert engine.model.get_sentence_embedding_dimension() == VFGConfig.EMBEDDING_DIMENSION
...
def test_add_doc_creates_embedding():
    ...
    assert engine.embeddings[0].shape[0] == VFGConfig.EMBEDDING_DIMENSION
