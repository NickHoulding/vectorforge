"""Centralized logging configuration for VectorForge MCP server.
...
def _sanitize_text_for_logging(
    text: str, max_len: int = MCPConfig.LOG_MAX_TEXT_LEN
) -> str:
...
def configure_logging(
    log_level: int = MCPConfig.LOG_LEVEL,
    log_file: str = MCPConfig.LOG_FILE,
    json_console: bool = MCPConfig.LOG_JSON_CONSOLE,
    max_bytes: int = MCPConfig.LOG_MAX_BYTES,
    backup_count: int = MCPConfig.LOG_BACKUP_COUNT,
) -> None:
    ...
    Args:
        log_level: Minimum logging level for the application (DEBUG, INFO, etc.).
            Defaults to MCPConfig.LOG_LEVEL if not specified.
        log_file: Path to log file (relative or absolute).
            Defaults to MCPConfig.LOG_FILE if not specified.
        json_console: Whether to output JSON to console (useful for production).
            Defaults to MCPConfig.LOG_JSON_CONSOLE if not specified.
        max_bytes: Maximum size per log file before rotation in bytes.
            Defaults to MCPConfig.LOG_MAX_BYTES if not specified.
        backup_count: Number of backup files to keep.
            Defaults to MCPConfig.LOG_BACKUP_COUNT if not specified.
    """
    ...
    app_logger = logging.getLogger("vectorforge_mcp")
